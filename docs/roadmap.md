...
- Логгер — выбрать/настроить (провайдер, уровни, куда пишет).
- Global Exception Handler — единая точка перехвата необработанных исключений в API-слое.
- Кастомные исключения (доменные/application-исключения, иерархия, маппинг на HTTP-статусы) — **требуют обсуждения с пользователем перед реализацией**, самостоятельно не проектировать.
- Ресурсы — состав и назначение ещё не уточнены, обсудить отдельно (возможно: строки сообщений об ошибках, локализация).
...
- Логгер: какой провайдер/подход (встроенный `ILogger`, Serilog и т.д.)?
- Кастомные исключения: какая иерархия и какая логика маппинга на HTTP-статусы нужна — ждём обсуждения перед реализацией.
- Ресурсы: что именно имеется в виду (строки ошибок, локализация, что-то ещё) — уточнить у пользователя.
...
Разговор поставлен на паузу: пользователь повторяет ООП перед сном.
